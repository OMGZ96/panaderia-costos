...
export const HistoryModal: React.FC<HistoryModalProps> = ({ isOpen, onClose, history, onClearHistory, onDeleteItem, onDeleteDay }) => {
  ...
  const handleExportExcel = () => {
    ...
    const flatData: any[] = history.flatMap((log): any => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[80vh] flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
              ...
              {sortedDates.map(date => {
                ...
                return (
                  <div key={date} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    {/* Day Header Row */}
                    <div className="p-4 flex flex-wrap gap-4 items-center justify-between cursor-pointer hover:bg-gray-50 transition-colors" onClick={() => toggleDate(date)}>
                      ...
                      <div className="flex items-center gap-6">
                        <div className="text-right">
                          ...
                          <p className={`font-mono font-bold ${dailyNet >= 0 ? 'text-green-600' : 'text-red-500'}`}>${dailyNet >= 0 ? '+' : ''}{dailyNet.toLocaleString('es-MX', { minimumFractionDigits: 2 })}</p>
                        </div>
                        <button
                              ...
                              return (
                                <React.Fragment key={log.id}>
                                  <tr className="bg-white hover:bg-gray-50 transition-colors">
                                    <td className="px-6 py-4 font-mono text-gray-500">{new Date(log.timestamp).toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit' })}</td>
                                    ...
                                    <td className="px-6 py-4 text-center">
                                      <div className="flex items-center justify-center gap-2">
                                        <button onClick={() => toggleRow(log.id)} className="text-xs font-medium text-blue-600 hover:text-blue-800 hover:underline">
                                          {isExpanded ? 'Ocultar' : 'Ver Detalles'}
                                        </button>
                                        <button onClick={() => onDeleteItem(log.id)} className="p-1 text-gray-300 hover:text-red-500 transition-colors" title="Eliminar registro">
                                          <TrashIcon />
                                        </button>
                                      </div>
                                    </td>
                                    ...
                                    <tr>
                                      <td colSpan={5} className="p-4 bg-gray-50 shadow-inner">
                                        <div className="bg-white border border-gray-200 rounded-lg overflow-hidden max-w-2xl mx-auto shadow-sm">
                                          ...
                                          {/* Cost breakdown for production logs */}
                                          {!isSale && (
                                            <div className="p-4">
                                              <div className="grid grid-cols-2 gap-2 text-sm">
                                                <div className="text-right font-medium text-gray-500">Subtotal Materia Prima:</div>
                                                <div className="text-right font-medium text-gray-700">${log.ingredientsUsed.reduce((acc, i) => acc + (i.cost || 0), 0).toFixed(2)}</div>
                                                <div className="text-right font-medium text-gray-500">+ Mano de Obra:</div>
                                                <div className="text-right font-medium text-gray-700">${(log.laborCost || 0).toFixed(2)}</div>
                                                <div className="text-right font-medium text-gray-500">+ Gastos Fijos:</div>
                                                <div className="text-right font-medium text-gray-700">${(log.fixedCosts || 0).toFixed(2)}</div>
                                                <div className="col-span-2 border-t border-gray-200 pt-2 text-right font-bold text-gray-800">Costo Total Producción:</div>
                                                <div className="text-right font-bold text-orange-600 text-sm">${log.totalCost.toFixed(2)}</div>
                                                {log.totalProfit !== undefined && (
                                                  <>
                                                    <div className="col-span-2 border-t border-gray-200 pt-2 text-right font-bold text-gray-800">Ganancia Neta:</div>
                                                    <div className={`text-right font-bold text-sm ${log.totalProfit >= 0 ? 'text-green-600' : 'text-red-600'}`}>${log.totalProfit.toFixed(2)}</div>
                                                  </>
                                                )}
                                              </div>
                                            </div>
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50 flex justify-between">
          <button onClick={onClearHistory} className="px-4 py-2 bg-white border border-red-200 text-red-600 hover:bg-red-50 hover:border-red-300 rounded-lg shadow-sm transition-all font-medium text-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2" disabled={history.length === 0}>
            <TrashIcon /> Borrar Todo el Historial
          </button>
          <div className="flex gap-2">
            <button onClick={handleExportExcel} disabled={history.length === 0} className="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg shadow-sm transition-all font-medium text-sm disabled:opacity-50 disabled:cursor-not-allowed">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
              </svg>
              Exportar Excel
            </button>
            <button onClick={onClose} className="px-6 py-2 bg-white border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 font-medium shadow-sm transition-all">
              Cerrar
            </button>
          </div>
        </div>
