...
import { Ingredient, ProductionData, CostAnalysis, ProductionLog, Buyer } from './types';
...
function App() {
  ...
  const totalProfitToday = useMemo(() => {
    return totalRevenueToday - totalProductionCostToday;
  }, [totalRevenueToday, totalProductionCostToday]);
  ...
  const handleFinalizeProduction = () => {
    ...
    const newLog: ProductionLog = {
      ...
      laborCost,
      fixedCosts,
      salePrice: averageSalePrice, // Snapshot of average sale price at this moment
      totalProfit: totalProfit, // Snapshot of projected total profit
  ...
  const handleTransferOrdersToSales = () => {
    // 1. Basic Check
    ...
    const confirmed = window.confirm(
      `¿Confirmar Transferencia a Ventas?\n\n` +
      `• Se moverán ${count} pedidos (${totalItems} panes) a la pestaña de Ventas.\n` +
      `• La lista de pedidos pendientes se vaciará automáticamente.\n\n` +
      `¿Desea continuar?`
    );
    
    if (confirmed) {
      // 5. Transfer logic (Regenerate IDs to prevent collision just in case)
  ...
  const handleFinalizeSales = () => {
    ...
    const newLog: ProductionLog = {
      ...
      type: 'sale',
      timestamp: new Date().toISOString(),
      yieldUnits: totalQuantitySold, // Total units sold
      totalCost: 0, // N/A for sales entry
      costPerUnit: averageSalePrice, // Average Sale Price
      totalProfit: totalRealSales, // Total Revenue for this sale batch
      ingredientsUsed: [], // Empty for sales
      buyersSnapshot: buyers.filter(b => b.quantity > 0) // Snapshot of buyers
    };
                  ...
                  >
                    ...
                    {activeTab === 'inventory' && (
                       <td className="px-2 md:px-4 py-3">
                         <input 
                            ...
                            className="w-full bg-white border border-gray-300 rounded px-2 py-1.5 text-gray-600 text-xs focus:outline-none focus:ring-2 focus:ring-blue-200 text-center font-medium"
                         />
                       </td>
                    )}
                    ...
                    <td className="px-2 md:px-4 py-3">
                      {activeTab === 'inventory' ? (
                        ...
                        <span className="text-gray-500 text-xs font-mono font-medium">${ing.pricePerUnit.toFixed(2)}</span>
                      )}
                    </td>
            ...
            {activeRightTab === 'orders' && (
              <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300 h-full flex flex-col">
                ...
                <div className="mt-auto shrink-0 pt-4 border-t border-gray-200 bg-gray-50">
                    ...
                    <button
                      onClick={handleTransferOrdersToSales}
            ...
            {activeRightTab === 'results' && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                ...
                <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
                  ...
                  <div className={`p-4 rounded-xl border ${totalProfitToday >= 0 ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                    <p className="text-xs text-gray-500 font-medium uppercase tracking-wider">Ganancia Neta Hoy</p>
                    <div className="mt-1 flex items-baseline">
                      <span className={`text-2xl font-bold ${totalProfitToday >= 0 ? 'text-green-700' : 'text-red-700'}`}>
                         ${totalProfitToday.toLocaleString('es-MX', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </span>
                    </div>
                    <p className="text-xs text-gray-400 mt-1">Ingresos - Costos Producción</p>
                  </div>
                </div>
                ...
                <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="text-xs font-bold text-gray-700 uppercase tracking-wide">Análisis Unitario (Lote Actual)</h3>
                  </div>
                  
                  <div className="space-y-3">
                     <div className="flex justify-between items-center py-2 border-b border-gray-50">
                        <span className="text-sm text-gray-600">Costo Producción Total</span>
                        <span className="font-bold text-gray-800">${analysis.totalProduction.toFixed(2)}</span>
                     </div>
                     <div className="flex justify-between items-center py-2 border-b border-gray-50">
                        <span className="text-sm text-gray-600">Costo Unitario (Prod)</span>
                        <span className="font-bold text-gray-800">${analysis.costPerUnit.toFixed(2)}</span>
                     </div>
                     
                     <div className="flex justify-between items-center py-2 border-b border-gray-50 bg-gray-50/50 px-2 rounded">
                        <span className="text-sm text-gray-600">Precio Promedio Venta</span>
                        <span className="font-bold text-purple-700">${averageSalePrice.toFixed(2)}</span>
                     </div>
                  </div>
                  
                  <div className="mt-4 grid grid-cols-2 gap-3">
                    <div className={`p-3 rounded-lg border text-center ${profitMargin > 0 ? 'bg-green-50 border-green-100' : 'bg-red-50 border-red-100'}`}>
                      <p className="text-[10px] uppercase font-bold text-gray-500">Margen / Unidad</p>
                      <p className={`text-lg font-bold ${profitMargin > 0 ? 'text-green-600' : 'text-red-600'}`}>
                        ${profitMargin.toFixed(2)}
                      </p>
                    </div>
                    <div className={`p-3 rounded-lg border text-center ${totalProfit > 0 ? 'bg-green-50 border-green-100' : 'bg-red-50 border-red-100'}`}>
                      <p className="text-[10px] uppercase font-bold text-gray-500">Ganancia Proy. Lote</p>
